...
namespace Procedural
{
    public class TextureGenerator : MonoBehaviour
    {
        ...
        public void GenerateTextures(TerrainData terrainData,ref TerrainType[,] terrainTypeMap, float maxAltitude)
        {
            ...
            float[,] grassMap = PerlinNoise.GenerateNoiseMap(width,height, width/plainFrequency, 9, plainPersistance, 2, new System.Random().Next(100000,1000000), new Vector2(0,0));
            terrainTypeMap = GenerateTerrainTypeMap(width, height, steepnessMap,grassMap);
            float[,,] splatMap = new float[width,height,nbTextures];

            float[,] forestSoilMap = PerlinNoise.GenerateNoiseMap(width,height, width/forestSoilFrequency, 9, forestSoilPersistance, 2, new System.Random().Next(100000,1000000), new Vector2(0,0));
